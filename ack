...
sub get_command_line_options {
    ...
    my $getopt_specs = {
        ...
        'C|context:i'           => sub { shift; my $val = shift; $opt{before_context} = $opt{after_context} = 
($val || 2) },
        'a|all-types'           => \$opt{all},
        'break!'                => \$opt{break},
        c                       => \$opt{count},
        ...
        'ignore-dirs=s'         => sub { shift; my $dir = remove_dir_sep( shift ); $ignore_dirs{$dir} = 
'--ignore-dirs' },
    ...
    for ( @_ ) {
        ...
        if ( $output_func ) {
            ...
            if ( $color && $is_match && $regex &&
                 s/$regex/Term::ANSIColor::colored( substr($_, $-[0], $+[0] - $-[0]), $ENV{ACK_COLOR_MATCH} 
)/eg ) {
...
sub filetype_setup {
    ...
    if ( !$filetypes_supported_set ) {
        for my $i ( keys %type_wanted ) {
            $type_wanted{$i} = 1 unless ( defined( $type_wanted{$i} ) || $i eq 'binary' || $i eq 'text' || $i 
eq 'skipped' );
        }
    }
    return;
        ...
        foreach my $pattern ( @{$argv} ) {
            ...
            if (@results == 0) {
                ...
                if ( $filter ) {
                    ...
                    @results = grep { not(Win32::File::GetAttributes($_, $attr) and $attr & $filter) } 
@results;
...
sub get_iterator {
    ...
    if ( $g_regex ) {
        $file_filter
            ...
            : $opt->{all} ? sub { $starting_point{ $File::Next::name } || ( $File::Next::name =~ /$g_regex/ && 
is_searchable( $_ ) ) }
            :               sub { $starting_point{ $File::Next::name } || ( $File::Next::name =~ /$g_regex/ && 
is_interesting( @_ ) ) }
            ;
    }
    else {
