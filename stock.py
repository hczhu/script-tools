...
BVPS = {
  ...
  # 招商银行, 2013年年报
  '招商银行': (
                265465 * 1.15   # 末期净资产加上估计的ROE带来的增长
                + 48764 # 加回贷款减值准备余额
                + (78 + 64) # 加回其他减值准备
                - (2996 + 9953) # 减去商誉和无形资产
                - 1.0 * (  # 不良贷款损失率
                  18332 # 已有不良总额
                  + 24603 * 0.8 # 从关注迁移到可疑的估计
                  + 126201 * 1.15 * ( # 内地公司贷款(加上增长估计)额外损失，包括以下三部分
                    0.86 / 100 # 股份行平均不良率
                    + 2 * 0.7 / 100 # 由GDP下行2个点带来
                  )
                  + 131061  * 20.0 / 100 # 房地产业贷款损失
                )
                - 117391 * 3 * 0.7 / 100 # 买入返售－信托受益权损失率按GDP下行带来的不良率计算
              ) * 10**6 / SHARES['招商银行']
              * 1.05, # 品牌溢价

  #中国银行，2013年年报
  '中国银行': (
                961477 * 1.10   # 末期净资产加上估计的ROE带来的增长
                + 168049 # 加回贷款减值准备余额
                - (1982 + 12819) # 减去商誉和无形资产
                - 1.0 * (  # 不良贷款损失率
                  73271 # 已有不良总额
                  + 189293 * 0.45 # 从关注迁移到可疑的估计
                ...
                - 147161 * 3 * 0.7 / 100 # 买入返售－信托受益权损失率按GDP下行带来的不良率计算
              ) * 10**6 / SHARES['中国银行'],
}
...
EPS = {
  ...
  # 招商银行, 2013年年报
  '招商银行': 10**6 * (
              29184 * 1.5 # 手续费和佣金净收入，按过去两年的平均增长估计
              + 4507 # 其非利息他净收入不变
              + (3507220 * 1.2) * 2.6 / 100 # 利息净收入 = 生息资产估计 * 净息差估计
