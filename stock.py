...
BVPS = {
  ...
  '招商银行': (
             265872 * 10**6 # 报告值
             - 9598 * 10**6 # 减去商誉
             - 2195807 * 10**6 * 2.1 / 100 * 0.8 # 不良率打折
             ) / SHARES['招商银行']
             * 1.1, # 品牌溢价

  #中国银行，2013年3季度财报，25%的不良贷款来自行政干预
  '中国银行': (
              (889259 * 10**6)
              * (1.0 + 15.0 / 100 / 4) # 加上4季度估计利润
              - (12307 + 1850) * 10**6 # 减去商誉和无形资产
              - 75353.74 * 10**8 * (1 + 1.3 / 100)  # 总资产估计，四季度大型银行环比1.3%
              * (2.1 / 100 * 1.2    # 减去估计的不良资产，相比股份行多出20%
                 + 0.08 * 0.1) # 房地产开发贷占8%
              ) / SHARES['中国银行'],
}
